...
func (a *Adaptor) DoResponse(c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo) (usage any, err *types.NewAPIError) {
	...
	if totalUsage.TotalTokens == 0 {
		...
		for _, payload := range a.payloads {
			totalUsage.PromptTokens += guardTemplateOverheadTokens
			for _, message := range payload.Messages {
				totalUsage.PromptTokens += service.CountTextToken(message.Content, info.UpstreamModelName)
			}
		}
		for _, verdict := range verdicts {
			totalUsage.CompletionTokens += service.CountTextToken(verdict, info.UpstreamModelName)
		}
