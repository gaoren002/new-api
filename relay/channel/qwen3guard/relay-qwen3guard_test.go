...
func TestGuardUsageAggregationIsBoundedAndAudited(t *testing.T) {
	info := &relaycommon.RelayInfo{}
	usage := &dto.Usage{}
	addGuardUsage(info, usage, 10, 4)
	require.Equal(t, 14, usage.TotalTokens, "a missing upstream total must not discard measured input/output usage")
	addGuardUsage(info, usage, math.MaxInt, math.MaxInt)
	require.Equal(t, common.MaxQuota, usage.PromptTokens)
	require.Equal(t, common.MaxQuota, usage.CompletionTokens)
	require.Equal(t, common.MaxQuota, usage.TotalTokens)
	require.NotNil(t, info.QuotaClamp)
	require.Equal(t, common.QuotaClampOverflow, info.QuotaClamp.Kind)
	addGuardUsage(info, usage, -20, -40)
	require.Equal(t, common.MaxQuota, usage.TotalTokens, "negative upstream counts cannot credit the request")
}
