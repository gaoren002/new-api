...
func getInviteCodeFromSession(session sessions.Session) string {
	...
	raw := session.Get("invite_code")
	value, ok := raw.(string)
	if !ok {
		return ""
	}
	return model.NormalizeInviteCode(value)
}

func persistInviteCodeToSession(session sessions.Session, inviteCode string) {
	if session == nil {
		return
	}
	...
	session.Set("invite_code", normalized)
}
