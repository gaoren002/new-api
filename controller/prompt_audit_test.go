...
func TestEvaluatePromptAuditBlocksBeforeRelay(t *testing.T) {
	...
	require.Equal(t, "request blocked by prompt audit: 越狱攻击", newAPIError.Error())
	require.NotContains(t, newAPIError.Error(), "latest input")
}

func TestEvaluatePromptAuditExemptUserRecordsWithoutBlocking(t *testing.T) {
	...
	storage.Mode = setting.PromptAuditModeBlocking
	storage.ExemptUserIDs = []int{42}
	...
	exempt := promptAuditGinContext(t, `{"messages":[{"role":"user","content":"latest input"}]}`)
	common.SetContextKey(exempt, constant.ContextKeyUserId, 42)
	require.Nil(t, evaluatePromptAudit(exempt, types.RelayFormatOpenAI, "default"))

	var events []model.PromptAuditEvent
	require.NoError(t, model.DB.Find(&events).Error)
	require.Len(t, events, 1)
	require.Equal(t, 42, events[0].UserId)
	require.Equal(t, "critical", events[0].Decision)
	require.Equal(t, "Block", events[0].Action)

	blocked := promptAuditGinContext(t, `{"messages":[{"role":"user","content":"latest input"}]}`)
	common.SetContextKey(blocked, constant.ContextKeyUserId, 7)
	newAPIError := evaluatePromptAudit(blocked, types.RelayFormatOpenAI, "default")
	require.NotNil(t, newAPIError)
	require.Equal(t, types.ErrorCodePromptBlocked, newAPIError.GetErrorCode())
	require.Equal(t, http.StatusForbidden, newAPIError.StatusCode)
}
...
func TestEvaluatePromptAuditExemptUserAllowsUnavailableBackend(t *testing.T) {
	previous := setting.GetPromptAuditConfig()
	previousPolicies := setting.PromptAuditGroupPolicies
	previousConfigJSON := setting.PromptAuditConfigJSON
	defer func() {
	...
	storage.Mode = setting.PromptAuditModeBlocking
	storage.ExemptUserIDs = []int{42}
	storage.Endpoints[0].BaseURL = "http://127.0.0.1:1"
	storage.Endpoints[0].Model = "guard-model"
	storage.Endpoints[0].TimeoutMS = 1000
	storage.Endpoints[0].InputLimit = 8000
	raw, err := json.Marshal(storage)
...
func TestEvaluatePromptAuditUsesGroupPolicy(t *testing.T) {
	...
	require.NotNil(t, newAPIError)
	require.Equal(t, types.ErrorCodePromptBlocked, newAPIError.GetErrorCode())
	require.Equal(t, "request blocked by prompt audit: 个人敏感信息", newAPIError.Error())
	context = promptAuditGinContext(t, `{"messages":[{"role":"user","content":"latest input"}]}`)
...
func TestUpdatePromptAuditConfigReportsOutOfRangeResourceLimits(t *testing.T) {
	usePromptAuditControllerTestDB(t)
	previousConfig := setting.PromptAuditConfigJSON
	t.Cleanup(func() { setting.PromptAuditConfigJSON = previousConfig })
	...
	tests := []struct {
		name            string
		mutate          func(*setting.PromptAuditStorageConfig)
		expectedMessage string
	}{
		{
			name: "worker count",
			mutate: func(config *setting.PromptAuditStorageConfig) {
				config.WorkerCount = 65
			},
			expectedMessage: "prompt audit worker count must be between 1 and 64",
		},
		{
			name: "endpoint input limit",
			mutate: func(config *setting.PromptAuditStorageConfig) {
				config.Endpoints[0].InputLimit = 2000001
			},
			expectedMessage: "prompt audit endpoint input limit must be between 128 and 2000000",
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			request := storage
			request.Endpoints = append([]setting.PromptAuditEndpoint(nil), storage.Endpoints...)
			test.mutate(&request)

			recorder := httptest.NewRecorder()
			context, _ := gin.CreateTestContext(recorder)
			context.Request = httptest.NewRequest(http.MethodPut, "/api/prompt-audit/config", strings.NewReader(string(promptAuditConfigUpdateBody(t, request, false))))
			common.SetContextKey(context, constant.ContextKeyUserId, 7)
			UpdatePromptAuditConfig(context)

			require.Equal(t, http.StatusBadRequest, recorder.Code)
			require.JSONEq(t, fmt.Sprintf(`{"success":false,"message":%q}`, test.expectedMessage), recorder.Body.String())
		})
	}
}
...
func promptAuditConfigUpdateBody(t *testing.T, storage setting.PromptAuditStorageConfig, clearToken bool) []byte {
	...
	raw, err := json.Marshal(map[string]any{
		...
		"exempt_user_ids":           storage.ExemptUserIDs,
