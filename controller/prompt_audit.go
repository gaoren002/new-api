...
func UpdatePromptAuditConfig(c *gin.Context) {
	...
	next := setting.PromptAuditStorageConfig{
		...
		ExemptUserIDs: setting.NormalizePromptAuditExemptUserIDs(request.ExemptUserIDs),
	...
	recordManageAudit(c, "prompt_audit.config_update", map[string]interface{}{
		...
		"exempt_user_count": len(next.ExemptUserIDs),
	})
...
func promptAuditPublicConfigFromStorage(storage setting.PromptAuditStorageConfig) promptAuditPublicConfig {
	public := promptAuditPublicConfig{
		...
		ExemptUserIDs: append([]int{}, storage.ExemptUserIDs...),
...
func evaluatePromptAuditPayload(c *gin.Context, relayFormat types.RelayFormat, group string, relayInfo *relaycommon.RelayInfo, body []byte, stage string) *types.NewAPIError {
	...
	if err != nil {
		if config.ExemptsUser(auditRequest.UserID) {
			logger.LogWarn(c, "prompt audit backend unavailable; exempt user allowed")
			return nil
		}
	...
	if decision != nil && decision.Blocked {
		if config.ExemptsUser(auditRequest.UserID) {
			logger.LogWarn(c, "prompt audit recorded without blocking exempt user")
			return nil
		}
		logger.LogWarn(c, "prompt audit blocked request")
		return types.NewErrorWithStatusCode(errors.New(service.PromptAuditBlockMessage(decision)), types.ErrorCodePromptBlocked, http.StatusForbidden, types.ErrOptionWithSkipRetry())
	}
	return nil
}
