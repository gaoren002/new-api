...
func CreatePromptAuditJobContext(ctx context.Context, job *PromptAuditJob, queueCapacity int) error {
	...
	return db.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).First(&admissionLock, 1).Error; err != nil {
			return err
		}
		var active int64
...
func ClaimPromptAuditJob(lease time.Duration) (*PromptAuditJob, bool, error) {
	...
	for attempt := 0; attempt < 8; attempt++ {
		...
		err := DB.Where("(status = ? OR (status = ? AND next_attempt_at <= ?))", PromptAuditJobQueued, PromptAuditJobRetry, now).
		...
		result := DB.Model(&PromptAuditJob{}).
			Where("id = ? AND (status = ? OR (status = ? AND next_attempt_at <= ?))", candidate.Id, PromptAuditJobQueued, PromptAuditJobRetry, now).
