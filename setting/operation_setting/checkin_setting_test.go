...
func TestCheckinQuotaConversionCannotOverflow(t *testing.T) {
	oldDisplayType := GetGeneralSetting().QuotaDisplayType
	t.Cleanup(func() { GetGeneralSetting().QuotaDisplayType = oldDisplayType })
	for _, displayType := range []string{QuotaDisplayTypeTokens, QuotaDisplayTypeUSD, QuotaDisplayTypeCNY} {
		GetGeneralSetting().QuotaDisplayType = displayType
		assert.Equal(t, math.MaxInt32, DisplayAmountToQuota(math.MaxFloat64))
		assert.Equal(t, 0, DisplayAmountToQuota(math.NaN()))
		assert.Equal(t, 0, DisplayAmountToQuota(-1))
	}
	assert.Equal(t, math.MaxInt32, CNYToQuota(math.MaxFloat64))
	assert.Equal(t, 0, CNYToQuota(math.NaN()))
}
