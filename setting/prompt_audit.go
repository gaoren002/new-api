...
const (
	...
	maxPromptAuditInputLimit         = 2000000
...
type PromptAuditStorageConfig struct {
	...
	Scanners               []string                          `json:"scanners"`
	FailClosed             bool                              `json:"fail_closed"`
	ExemptUserIDs          []int                             `json:"exempt_user_ids,omitempty"`
...
func (config PromptAuditConfig) ExemptsUser(userID int) bool {
	if userID <= 0 {
		return false
	}
	for _, id := range config.ExemptUserIDs {
		if id == userID {
			return true
		}
	}
...
func promptAuditActiveConfig(storage PromptAuditStorageConfig, group string) PromptAuditConfig {
	...
	config := PromptAuditConfig{
		...
		ExemptUserIDs: append([]int(nil), storage.ExemptUserIDs...),
	}
...
func normalizePromptAuditStorageConfig(config *PromptAuditStorageConfig) {
	...
	config.ExemptUserIDs = NormalizePromptAuditExemptUserIDs(config.ExemptUserIDs)
...
func ValidatePromptAuditStorageConfig(config PromptAuditStorageConfig) error {
	...
	if len(config.ExemptUserIDs) > maxPromptAuditExemptUserIDs {
		return fmt.Errorf("prompt audit exempt user list must not contain more than %d IDs", maxPromptAuditExemptUserIDs)
	}
	for _, userID := range config.ExemptUserIDs {
		if userID <= 0 {
			return fmt.Errorf("prompt audit exempt user IDs must be positive")
		}
	}
	...
	for _, endpoint := range config.Endpoints {
		...
		if endpoint.InputLimit < 128 || endpoint.InputLimit > maxPromptAuditInputLimit {
			return fmt.Errorf("prompt audit endpoint input limit must be between 128 and %d", maxPromptAuditInputLimit)
		}
		if endpoint.Enabled {
			enabled++
		}
...
func NormalizePromptAuditExemptUserIDs(ids []int) []int {
	if len(ids) == 0 {
		return nil
	}
