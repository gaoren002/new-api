...
func TestPromptAuditExemptUserIDs(t *testing.T) {
	require.Equal(t, []int{1, 3}, NormalizePromptAuditExemptUserIDs([]int{3, 0, -1, 1, 3}))
	require.Nil(t, NormalizePromptAuditExemptUserIDs(nil))
	require.Nil(t, NormalizePromptAuditExemptUserIDs([]int{0, -3}))

	config := PromptAuditConfig{ExemptUserIDs: []int{7, 9}}
	require.True(t, config.ExemptsUser(7))
	require.False(t, config.ExemptsUser(8))
	require.False(t, config.ExemptsUser(0))

	storage := DefaultPromptAuditStorageConfig()
	storage.ExemptUserIDs = []int{0, 3}
	require.EqualError(t, ValidatePromptAuditStorageConfig(storage), "prompt audit exempt user IDs must be positive")
	storage.ExemptUserIDs = []int{3, 1}
	require.NoError(t, ValidatePromptAuditStorageConfig(storage))

	previous := PromptAuditConfigJSON
	defer func() { PromptAuditConfigJSON = previous }()
	storage.ExemptUserIDs = []int{9, 0, 2, 9}
	raw, err := json.Marshal(storage)
	require.NoError(t, err)
	PromptAuditConfigJSON = string(raw)
	loaded, err := GetPromptAuditStorageConfig()
	require.NoError(t, err)
	require.Equal(t, []int{2, 9}, loaded.ExemptUserIDs)
	require.True(t, GetPromptAuditConfig().ExemptsUser(2))
	require.False(t, GetPromptAuditConfig().ExemptsUser(1))
}
