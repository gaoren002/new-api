...
const (
	promptAuditPIICategory = "pii"
	// promptAuditWithheldPreview matches the marker promptAuditPreview already emits
	// for prompts too short to preview, so stored rows stay uniform.
	promptAuditWithheldPreview = "***"
)

// promptAuditWithholdsPromptText reports whether a decision was driven by the PII
// scanner. Such a prompt is known to carry personal data, so no part of it may be
// persisted: the audit trail keeps only the hash, the categories, and the metadata.
func promptAuditWithholdsPromptText(decision *PromptAuditDecision) bool {
	if decision == nil {
		return false
	}
	for _, category := range decision.Categories {
		if category == promptAuditPIICategory {
			return true
		}
	}
	for _, scanner := range decision.MatchedScanners {
		if scanner == promptAuditPIICategory {
			return true
		}
	}
	return false
...
const promptAuditBlockedMessage = "request blocked by prompt audit"

// PromptAuditBlockMessage is the user-visible 403 text. It names the matched
// scanner labels so the caller can tell why the request was refused, but never
// includes prompt text, previews, or unknown-category hashes.
func PromptAuditBlockMessage(decision *PromptAuditDecision) string {
	reasons := promptAuditBlockReasons(decision)
	if len(reasons) == 0 {
		return promptAuditBlockedMessage
	}
	return promptAuditBlockedMessage + ": " + strings.Join(reasons, ", ")
}

func promptAuditBlockReasons(decision *PromptAuditDecision) []string {
	if decision == nil {
		return nil
	}
	seen := map[string]struct{}{}
	var reasons []string
	addKnown := func(ids []string) {
		for _, category := range ids {
			definition, ok := promptAuditScannerDefinitions[category]
			if !ok || definition.LabelZH == "" {
				continue
			}
			...
			reasons = append(reasons, definition.LabelZH)
		}
	}
	addKnown(decision.MatchedScanners)
	addKnown(decision.Categories)
	if len(reasons) == 0 && len(decision.UnknownCategories) > 0 {
		return []string{"未知高风险分类"}
	}
	return reasons
}
