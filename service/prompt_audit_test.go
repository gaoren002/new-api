...
func TestPromptAuditBlockMessageUsesChineseCategoryLabels(t *testing.T) {
	require.Equal(t, "request blocked by prompt audit", PromptAuditBlockMessage(nil))
	require.Equal(t, "request blocked by prompt audit", PromptAuditBlockMessage(&PromptAuditDecision{Blocked: true}))
	require.Equal(t, "request blocked by prompt audit: 个人敏感信息", PromptAuditBlockMessage(&PromptAuditDecision{
		Blocked: true, MatchedScanners: []string{"pii"},
	}))
	require.Equal(t, "request blocked by prompt audit: 越狱攻击, 暴力", PromptAuditBlockMessage(&PromptAuditDecision{
		Blocked: true, MatchedScanners: []string{"jailbreak", "violent"}, Categories: []string{"jailbreak"},
	}))
	require.Equal(t, "request blocked by prompt audit: 未知高风险分类", PromptAuditBlockMessage(&PromptAuditDecision{
		Blocked: true, UnknownCategories: []string{"unknown:abcd"},
	}))
	require.NotContains(t, PromptAuditBlockMessage(&PromptAuditDecision{
		Blocked: true, MatchedScanners: []string{"pii"},
	}), "110101")
}
