...
func promptAuditProcessJob(parent context.Context, job *model.PromptAuditJob) {
	...
	snapshot.RedactedPreview = promptAuditPreview(snapshot.FullPrompt)
...
// promptAuditRedactedSnapshotJSON strips every prompt-derived field before the
// snapshot reaches the job row. The queue only carries routing metadata; the
// prompt itself lives in the transient Redis payload and is reconstructed, along
// with its preview, when the worker builds the event.
...
func promptAuditEventFromSnapshot(snapshot PromptAuditSnapshot, decision *PromptAuditDecision, configVersion int64, jobID *int64) *model.PromptAuditEvent {
	...
	if promptAuditWithholdsPromptText(decision) {
		event.FullPrompt, event.RedactedPreview = "", promptAuditWithheldPreview
	}
	return event
}
