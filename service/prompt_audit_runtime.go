...
}{enqueueSlots: make(chan struct{}, promptAuditEnqueueSlots), probes: map[string]PromptAuditProbeResult{}}
...
func promptAuditWorker(ctx context.Context, workerID int) {
	...
	for {
		...
		for {
			...
			job, claimed, err := model.ClaimPromptAuditJob(promptAuditJobLease)
			if err != nil {
