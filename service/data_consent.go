...
func DataConsentStateForTaskBillingContext(billingContext *model.TaskBillingContext) DataConsentBillingInfo {
	if billingContext == nil {
		// Historical tasks without a consent snapshot were submitted without
		// this adjustment. Current settings must not change their settlement.
		return DataConsentBillingInfo{Status: dto.DataConsentStatusDisabled, Multiplier: 1}
	}
	info := DataConsentBillingInfo{
	...
	if info.Multiplier <= 0 || math.IsNaN(info.Multiplier) || math.IsInf(info.Multiplier, 0) {
		info.Multiplier = 1
	}
	return info
}
...
func ApplyDataConsentMultiplier(relayInfo *relaycommon.RelayInfo, quota int) int {
	adjusted, clamp := ApplyDataConsentMultiplierByInfoChecked(quota, DataConsentStateForRelayInfo(relayInfo))
...
func ApplyDataConsentMultiplierToPriceData(relayInfo *relaycommon.RelayInfo) {
	...
	if relayInfo.PriceData.QuotaToPreConsume != 0 {
		relayInfo.PriceData.QuotaToPreConsume = ApplyDataConsentMultiplier(relayInfo, relayInfo.PriceData.QuotaToPreConsume)
	}
	if relayInfo.PriceData.Quota != 0 {
		relayInfo.PriceData.Quota = ApplyDataConsentMultiplier(relayInfo, relayInfo.PriceData.Quota)
	}
}
...
func ApplyDataConsentMultiplierByInfo(quota int, info DataConsentBillingInfo) int {
	adjusted, _ := ApplyDataConsentMultiplierByInfoChecked(quota, info)
	return adjusted
}

// ApplyDataConsentMultiplierByInfoChecked applies the frozen task/request
// adjustment without allowing an oversized charge to overflow or lose its audit.
func ApplyDataConsentMultiplierByInfoChecked(quota int, info DataConsentBillingInfo) (int, *common.QuotaClamp) {
	if !info.Enabled || quota == 0 || info.Multiplier == 1 {
		return quota, nil
	}
	if info.Multiplier <= 0 || math.IsNaN(info.Multiplier) || math.IsInf(info.Multiplier, 0) {
		common.SysError("invalid data consent billing multiplier; using the unadjusted charge")
		return quota, nil
	}
	adjusted, clamp := common.QuotaRoundChecked(float64(quota) * info.Multiplier)
