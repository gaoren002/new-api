...
func TestBlockingPromptAuditWithholdsPromptTextOnPIIHits(t *testing.T) {
	previousDB := model.DB
	db, err := gorm.Open(sqlite.Open(fmt.Sprintf("file:prompt-audit-pii-%d?mode=memory&cache=shared", time.Now().UnixNano())), &gorm.Config{})
	require.NoError(t, err)
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, _ *http.Request) {
		_, _ = writer.Write([]byte(`{"choices":[{"message":{"content":` + mustJSONPromptAudit(t, verdict) + `}}]}`))
	}))
	defer server.Close()
	config := promptAuditTestConfig(server.URL)
	config.Mode = setting.PromptAuditModeBlocking

	const prompt = "please mail the contract to Zhang Wei, national id 110101199001011234"
	request := PromptAuditRequest{
		RequestID: "req-pii", UserID: 7, Username: "alice", Protocol: "openai_chat_completions",
		Body: []byte(`{"messages":[{"role":"user","content":"` + prompt + `"}]}`),
	}
	decision, err := EvaluatePromptAuditRequest(context.Background(), config, request)
	require.NoError(t, err)
	require.True(t, decision.Blocked)
	require.Equal(t, []string{"pii"}, decision.MatchedScanners)

	var piiEvent model.PromptAuditEvent
	require.NoError(t, db.Where("request_id = ?", "req-pii").First(&piiEvent).Error)
	require.Empty(t, piiEvent.FullPrompt)
	require.Equal(t, "***", piiEvent.RedactedPreview)
	require.NotEmpty(t, piiEvent.PromptHash)
	require.Equal(t, 7, piiEvent.UserId)
	require.Contains(t, piiEvent.CategoriesJSON, "pii")

	var piiJob model.PromptAuditJob
	require.NoError(t, db.Where("request_id = ?", "req-pii").First(&piiJob).Error)
	require.NotContains(t, piiJob.SnapshotJSON, "Zhang Wei")

	verdict = "Safety: Unsafe\nCategories: Violent"
	request.RequestID = "req-violent"
	decision, err = EvaluatePromptAuditRequest(context.Background(), config, request)
	require.NoError(t, err)
	require.True(t, decision.Blocked)

	var violentEvent model.PromptAuditEvent
	require.NoError(t, db.Where("request_id = ?", "req-violent").First(&violentEvent).Error)
	require.Equal(t, prompt, violentEvent.FullPrompt)
	require.NotEqual(t, "***", violentEvent.RedactedPreview)
}
