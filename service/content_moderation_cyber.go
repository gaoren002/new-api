...
func firstCyberPolicyToken(payload []byte, paths ...string) int {
...
func MarkCyberPolicy(c *gin.Context, mark CyberPolicyMark) {
	...
	if mark.UpstreamInputTokens <= 0 && mark.UpstreamOutputTokens <= 0 {
		mark.UpstreamInputTokens, mark.UpstreamOutputTokens = ExtractCyberPolicyUsage([]byte(mark.Body))
	}
