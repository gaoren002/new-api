...
function cloneConfig(config: PromptAuditConfig): PromptAuditConfig {
  ...
  cloned.exempt_user_ids = Array.isArray(cloned.exempt_user_ids)
    ? cloned.exempt_user_ids
    : []
...
export function PromptAuditSection({ groupRatio }: PromptAuditSectionProps) {
  ...
  const [exemptUserDraft, setExemptUserDraft] = useState('')
  ...
  const save = async () => {
    ...
    try {
      const saved = await updatePromptAuditConfig({
        ...
        fail_closed: true,
        exempt_user_ids: config.exempt_user_ids ?? [],
  ...
  const addExemptUser = () => {
    if (!config) return
    const userID = Number(exemptUserDraft.trim())
    if (!Number.isInteger(userID) || userID <= 0) {
      toast.error(t('Enter a valid user ID'))
      return
    }
    const current = config.exempt_user_ids ?? []
    if (current.includes(userID)) {
      toast.error(t('This user ID is already exempt'))
      return
    }
    if (current.length >= 4096) {
      toast.error(t('Exempt user list is full'))
      return
    }
    patchConfig({
      exempt_user_ids: [...current, userID].sort((left, right) => left - right),
    })
    setExemptUserDraft('')
  }
          ...
          <section className='grid gap-5 border-b pb-6 md:grid-cols-2'>
            ...
            <div className='space-y-3 md:col-span-2'>
              <div className='space-y-1'>
                <h3 className='text-sm font-semibold'>{t('Exempt users')}</h3>
                <p className='text-muted-foreground text-xs'>
                  {t(
                    'In blocking mode, exempt users are still scanned and recorded, but their requests are not blocked.'
                  )}
                </p>
              </div>
              <div className='flex flex-wrap items-center gap-2'>
                <Input
                  ...
                  value={exemptUserDraft}
                  onChange={(event) => setExemptUserDraft(event.target.value)}
                  onKeyDown={(event) => {
                    if (event.key === 'Enter') {
                ...
                >
                  <Plus className='size-4' /> {t('Add user ID')}
                </Button>
              </div>
              {(config.exempt_user_ids ?? []).length > 0 ? (
                <div className='flex flex-wrap gap-2'>
                  {(config.exempt_user_ids ?? []).map((userID) => (
                    <Badge
                      key={userID}
                      variant='secondary'
